...
export function ChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "你好，我是即答AI客服演示助手。你可以问我适用场景、价格、搭建周期，也可以直接说你的行业和每天咨询量，我帮你判断是否适合先试跑。",
    },
  ]);
      ...
      setMessages((current) =>
        current.map((item) =>
          item.id === assistantId && !item.content
            ? { ...item, content: "这次连接没有成功。你可以稍后重试，或在案例页留下联系方式。" }
            : item,
        ),
      );
    } finally {
  ...
  useEffect(() => {
    ...
    if (question) {
      ...
      window.setTimeout(() => sendMessage(question), 200);
    }
  }, []);
  ...
  return (
    <main className="flex min-h-screen flex-col bg-[#f7f8ff] text-brand-ink dark:bg-[#0f1024] dark:text-white">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur dark:border-white/10 dark:bg-[#0f1024]/95">
        <div className="mx-auto flex h-14 max-w-3xl items-center justify-between px-3 min-[380px]:px-4">
          <a href="/case" className="flex min-w-0 items-center gap-2 font-black text-brand">
          ...
          <span className="flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-800">
      ...
      <section className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-3 pb-[7.5rem] pt-4 min-[380px]:px-4">
        <div className="mb-4 rounded-2xl bg-brand-ink p-4 text-white dark:bg-[#080914]">
          <p className="text-sm font-black text-indigo-200">案例体验</p>
          <h1 className="mt-2 text-2xl font-black leading-tight min-[420px]:text-3xl">
            直接问即答AI客服
          </h1>
          <p className="mt-2 text-sm leading-6 text-white/78">
            建议先问一个具体问题，或者直接告诉我你的行业、每天咨询量和当前客服渠道。
          </p>
        </div>
        ...
        <div className="flex flex-1 flex-col gap-3">
          {messages.map((message) => (
            ...
            >
              <div
                className={`max-w-[88%] whitespace-pre-wrap rounded-2xl px-4 py-3 text-sm leading-7 min-[420px]:text-base ${
                  message.role === "user"
      ...
      <form
        onSubmit={onSubmit}
        className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/96 px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 backdrop-blur dark:border-white/10 dark:bg-[#0f1024]/96 min-[380px]:px-4"
      >
        <div className="mx-auto flex max-w-3xl gap-2">
          <input
            ...
            disabled={loading}
            placeholder="输入你的问题，例如：我做女装电商适合吗？"
